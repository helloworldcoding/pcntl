...
// 把数据加入消息队列,默认数据会被序列化
msg_send($msgQueue,2,'hahha,1');
msg_send($msgQueue,4,'ooooo,2');
msg_send($msgQueue,2,'ooooo,3');
msg_send($msgQueue,1,'bbbbbb,4');
msg_send($msgQueue,1,'xxxxx,3');
msg_send($msgQueue,3,'xxxxx,4');

// 从消息队列中读取一条消息

msg_receive($msgQueue,-3, $message_type, 1024, $message3,true,MSG_IPC_NOWAIT);
sleep(1);
//msg_receive($msgQueue,0, $message_type, 1024, $message3);
msg_receive($msgQueue,1, $message_type, 1024, $message1);
msg_receive($msgQueue,2, $message_type, 1024, $message2);
